/* FieldTech.jsx — IIFE-wrapped browser mirror of src/lib/commandSprites/FieldTech.tsx
 *
 * Forward Observer Field Tech (Variant C). Family member of Marine + Engineer.
 *
 *   - Same 5-tone armor ramp (Steel default), same gunmetal device ramp,
 *     same 1-px dark outline pass, same soft pixel ground-shadow ellipse.
 *   - Accent zones (TCZ): binocular lenses, pauldron outer edges, chest
 *     indicator strip, belt buckle. (§7.3 calls out pauldron strip +
 *     scanner display; on this variant the binoculars ARE the scanner.)
 *   - Distinguishing features: wide horizontal binocular bar over the
 *     visor zone (rows 9-11); vertical antenna spike rising from a radio
 *     backpack (col 28, rows 1-5); short scoped carbine on the right hip.
 *
 * Animation vocabulary — Forward-Observer specific overlays on the
 * family's shared motion grammar:
 *   idle                — body sway ±1px X (Marine pattern)
 *   thinking            — raise one binocular eyebrow + scratch chin
 *                         (no head tilt, no antenna twitch — just the
 *                         brow + hand. Same vocabulary as Marine.)
 *   tool_running        — scans a foreign object on the ground next to
 *                         her — the binoculars emit a left-to-right scan
 *                         beam that sweeps across the artifact
 *   streaming           — speech bubble + radio dots rising from antenna
 *                         + antenna pulses (no head bob — head stays put)
 *   awaiting_permission — patient jump + right hand wave overhead
 *                         (matches Marine + Engineer family pattern)
 *   done                — body snap-up, lens brightens, one-shot
 *   error               — body tilt + flashing red lens + drooped antenna
 *   spawning            — straight drop-in from above with impact bounce
 *   deploying           — same red-button kit as Marine + Engineer;
 *                         right arm reaches over and presses the dome
 */
...
/* Tool running — BEAM TRAPEZOID emitting from her visor (8 cols wide,
 * matching the visor strip cols 20-27) down to the artifact area
 * (cols 33-40, 8 wide). The top of the beam stays anchored to her
 * visor (transform-origin top); the WHOLE beam scaleY's around that
 * top so the bottom edge sweeps UP↔DOWN over the artifact's row
 * range. Rendered as a hollow outline (left + right diagonals + base
 * edge — top edge is implicit at the visor) so the unit's body shows
 * through. Pulses opacity in lockstep with the sweep. */
const SCAN_BEAM_TRIANGLE = [
  // Left diagonal: visor (20, 12) → base (33, 42)
  { x: 20, y: 12, w: 1, h: 2, c: "v" },
  { x: 21, y: 14, w: 1, h: 2, c: "v" },
  { x: 22, y: 16, w: 1, h: 3, c: "v" },
  { x: 23, y: 19, w: 1, h: 2, c: "v" },
  { x: 24, y: 21, w: 1, h: 2, c: "v" },
  { x: 25, y: 23, w: 1, h: 3, c: "v" },
  { x: 26, y: 26, w: 1, h: 2, c: "v" },
  { x: 27, y: 28, w: 1, h: 2, c: "v" },
  { x: 28, y: 30, w: 1, h: 3, c: "v" },
  { x: 29, y: 33, w: 1, h: 2, c: "v" },
  { x: 30, y: 35, w: 1, h: 2, c: "v" },
  { x: 31, y: 37, w: 1, h: 3, c: "v" },
  { x: 32, y: 40, w: 1, h: 2, c: "v" },
  // Right diagonal: visor (27, 12) → base (40, 42)
  { x: 27, y: 12, w: 1, h: 2, c: "v" },
  { x: 28, y: 14, w: 1, h: 2, c: "v" },
  { x: 29, y: 16, w: 1, h: 3, c: "v" },
  { x: 30, y: 19, w: 1, h: 2, c: "v" },
  { x: 31, y: 21, w: 1, h: 2, c: "v" },
  { x: 32, y: 23, w: 1, h: 3, c: "v" },
  { x: 33, y: 26, w: 1, h: 2, c: "v" },
  { x: 34, y: 28, w: 1, h: 2, c: "v" },
  { x: 35, y: 30, w: 1, h: 3, c: "v" },
  { x: 36, y: 33, w: 1, h: 2, c: "v" },
  { x: 37, y: 35, w: 1, h: 2, c: "v" },
  { x: 38, y: 37, w: 1, h: 3, c: "v" },
  { x: 39, y: 40, w: 1, h: 2, c: "v" },
  // Base edge — on the artifact (row 42, cols 33-40, 8 wide)
  { x: 33, y: 42, w: 8, h: 1, c: "V" },
];
 ...
 *
 * The beam is a WIDE TRAPEZOID emitting from her visor (8 cols wide,
 * matching the visor strip) down to the artifact. The top of the
 * beam is anchored at the visor (transform-origin = top-left of
 * bounding box = (20, 12)) and the whole group scaleY's around that
 * top, so the BOTTOM edge of the beam sweeps UP↔DOWN across the
 * artifact's row range. The visor end is the pivot — stays stationary.
 */
@keyframes ft-scan-beam-sweepY {
  0%, 100% { transform: scaleY(0.93); }   /* base at row ≈40 (top of artifact) */
  50%      { transform: scaleY(1.07); }   /* base at row ≈44 (bottom of artifact) */
}
...
/* Pivot the beam from the top-left of its bounding box (row 12 at the visor). */
.ft-scanBeam {
  transform-box: fill-box;
  transform-origin: 0% 0%;
}
.ft-state-tool_running .ft-scanBeam   { animation: ft-scan-beam-sweepY 1400ms linear infinite, ft-scan-beam-pulse 360ms linear infinite; }
.ft-state-tool_running .ft-scanObject { animation: ft-scan-object-bright 1400ms linear infinite; }
...
@media (prefers-reduced-motion: reduce) {
  ...
  .ft-state-tool_running .ft-scanBeam { opacity: 1; transform: scaleY(1); }
...
Object.assign(window, { FieldTech, ACCENTS, ARMOR_TEMPLATES, GUN_TEMPLATES });
