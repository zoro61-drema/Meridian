...
impl AppState {
  ...
  pub fn launched(app_delegate: &Object) {
    ...
    unsafe {
      ...
      // activateIgnoringOtherApps was deprecated in macOS 14 and throws an
      // NSException on macOS 26+. Use the replacement activate() instead.
      let _ = get_aux_state_mut(app_delegate).activate_ignoring_other_apps;
      ns_app.activate();
    ...
    HANDLER.waker().start();
    HANDLER.set_in_callback(true);
    // On macOS 26+, Tauri's dev-mode code calls setApplicationIconImage which throws an
    // NSException (zero-sized NSImage). Catch and discard it — the only consequence is the
    // dock icon won't be set in dev mode, which is non-critical.
    ...
    if let Err(exc) = catch_result {
      eprintln!(
        "tao: non-fatal ObjC exception during launch init (macOS 26 compat): {:?}",
        exc
      );
    }
    HANDLER.set_in_callback(false);
