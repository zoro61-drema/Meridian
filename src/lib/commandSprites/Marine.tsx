...
// The component uses implicit-any helper functions and untyped pixel-stencil
// arrays that don't survive our strict tsconfig — but it's hand-authored
// pixel art and re-typing every helper would diverge from the design source.
// The public surface (UnitProps + exported types below) is correctly typed
// and that's what the rest of the app sees.
/* src/lib/commandSprites/Marine.tsx — Veteran Scout marine, self-contained. */
...
// --- Thinking: bent arm pose with hand at chin (stays connected to shoulder)
const THINKING_ARM = [
  { x: 30, y: 18, w: 2, h: 1, c: "3" },
  { x: 30, y: 19, w: 2, h: 1, c: "3" },
  { x: 29, y: 19, w: 1, h: 1, c: "3" },
  { x: 28, y: 18, w: 1, h: 1, c: "3" },
  { x: 27, y: 17, w: 1, h: 1, c: "3" },
  { x: 26, y: 16, w: 1, h: 1, c: "3" },
  { x: 25, y: 15, w: 1, h: 1, c: "3" },
  { x: 25, y: 14, w: 1, h: 1, c: "3" },
  { x: 24, y: 14, w: 1, h: 1, c: "h" },
  { x: 23, y: 13, w: 2, h: 1, c: "h" },
];

// --- Thinking: raised left eyebrow — only visible during scratch beats
const THINKING_BROW = [
  { x: 20, y: 9, w: 3, h: 1, c: "v" },
];

// --- Tool running: squinting visor (right eye open, left eye closed)
const TOOL_RUNNING_VISOR = [
  { x: 20, y: 10, w: 3, h: 1, c: "k" },
  { x: 20, y: 11, w: 3, h: 1, c: "k" },
  { x: 26, y: 10, w: 1, h: 1, c: "v" },
  { x: 26, y: 11, w: 1, h: 1, c: "V" },
];
...
/* ===== thinking: bent arm visible (connected shoulder → hand at chin) + scratch jiggle ===== */
...
/* Hide the idle-pose right arm during thinking; show the bent-arm overlay instead. */
.mr-state-thinking .mr-rightArm { visibility: hidden; }
.mr-state-thinking .mr-thinkingArm {
  animation: mr-think-pose 1200ms linear infinite, mr-think-jiggle 1200ms linear infinite;
}
/* Eyebrow only visible during scratch beats — pulses with the jiggle */
@keyframes mr-think-brow-flash {
  0%, 13.99% { opacity: 0; }
  14%, 22%   { opacity: 1; }
  22.01%, 27.99% { opacity: 0; }
  28%, 36%   { opacity: 1; }
  36.01%, 41.99% { opacity: 0; }
  42%, 50%   { opacity: 1; }
  50.01%, 100% { opacity: 0; }
}
.mr-state-thinking .mr-thinkBrow {
  animation: mr-think-brow-flash 1200ms linear infinite;
}
...
/* Hide idle arms; show gesticulating overlays */
.mr-state-streaming .mr-leftArm, .mr-state-streaming .mr-rightArm { visibility: hidden; }
.mr-state-streaming .mr-streamLeft  { animation: mr-stream-leftArm 1000ms linear infinite; transform-origin: 15px 22px; }
.mr-state-streaming .mr-streamRight { animation: mr-stream-rightArm 1000ms linear infinite; transform-origin: 33px 22px; }
...
@keyframes mr-deploy-arm {
  ...
  15%, 64.99% { transform: translate(3px,-10px); }
  65%, 100% { transform: translate(0,0); }
}
@keyframes mr-deploy-spark {
  0%, 24.99% { opacity: 0; transform: translate(0,0); }
  25%, 49.99% { opacity: 1; transform: translate(2px,-2px); }
  50%, 100% { opacity: 0; transform: translate(4px,-4px); }
}
.mr-tr-deploying .mr-rightArm { animation: mr-deploy-arm 800ms linear forwards; }
.mr-tr-deploying .mr-deploySpark { animation: mr-deploy-spark 800ms linear forwards; }
...
@media (prefers-reduced-motion: reduce) {
  ...
  .mr-state-thinking .mr-rightArm { visibility: hidden; }
  .mr-state-thinking .mr-thinkingArm { opacity: 1; }
  .mr-state-thinking .mr-head, .mr-state-thinking .mr-visor { transform: translateX(-1px); }
...
export const Marine: React.FC<UnitProps> = ({
  state,
  transient,
  accent,
  size = 64,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {state === "thinking" && (
          <>
            <g className="mr-g mr-thinkingArm">
              {THINKING_ARM.map((p, i) => (
                <rect key={i} x={p.x} y={p.y} width={p.w} height={p.h}
                  fill={resolveColor(p.c, ramps, errorMode)} />
              ))}
            </g>
        ...
        {state === "streaming" && (
          <>
            <g className="mr-g mr-streamLeft">
              {STREAM_LEFT_ARM.map((p, i) => (
                <rect key={i} x={p.x} y={p.y} width={p.w} height={p.h}
                  fill={resolveColor(p.c, ramps, errorMode)} />
              ))}
            </g>
            <g className="mr-g mr-streamRight">
              {STREAM_RIGHT_ARM.map((p, i) => (
                <rect key={i} x={p.x} y={p.y} width={p.w} height={p.h}
                  fill={resolveColor(p.c, ramps, errorMode)} />
              ))}
            </g>
        ...
        {transient === "deploying" && (
          <g className="mr-g mr-deploySpark">
